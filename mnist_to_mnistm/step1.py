...
#netG.load_state_dict(torch.load('modelp/210_net_netGtp.pkl')) 
netG.cuda()
print(netG)
...
G_solver = optim.SGD(G_params, lr = learning_rate, momentum = 0.9)
...
for epoch in range(num_epochs):
  ...
  for step, (s_img, s_label, s_name) in enumerate(training_data_loader_s):
   ...
   if j < len(training_data_loader_t)-1:
    ...
    t_img, t_label, t_name = t_dataset.next()
    t_img = Variable(t_img.cuda())
    ...
    domain_t_label = torch.LongTensor(mb_size/2).zero_() + 1
    domain_t_label.cuda()
    domain_s_label = torch.LongTensor(mb_size/2).zero_()
    domain_s_label.cuda()
    domain_label = Variable(torch.cat((domain_t_label, domain_s_label), 0)).cuda()
    ...
    loss = nn.NLLLoss()
    G_loss_anchor = loss(m(pred_label_s), s_label)
    ...
    G_loss_t = loss(m(pred_label_t), t_label)	

    writer.add_scalar('eval/G_loss_t', G_loss_t.data[0], i)
    ...
    if (i) % 100 == 0:
        ...
        img = utils.make_grid(imgvis.data, nrow = 10, padding = 2, normalize=True, scale_each=False, pad_value=1)   
        ...
        plt.savefig('emrEuse.png')
        p1 = imread('emrEuse.png')
    ...
    if (step) % 10 == 0:
        print ('reverse_gradientuse:Epoch [%d/%d],Step [%d/%d], G_Loss: %.4f' %(epoch+1, num_epochs, step, len(training_data_loader_s)-1, G_loss.data[0] ))
  ...
  if acc > best_acc:
    ...
    netE.save_network(netE, 'netE', epoch, 'modelrgm/')
    
    netG.save_network(netG, 'netG', epoch, 'modelrgm/')
