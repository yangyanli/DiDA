...
netSEn.load_state_dict(torch.load('modelitergod/976_net_netEgod1v16.pkl'))
netSEn.cuda()
print(netSEn)

netG = NetG(input_nc, output_nc).eval()
netG.load_state_dict(torch.load('modelitergod/976_net_netGgod1v16.pkl')) 
netG.cuda()
print(netG)
...
for epoch in range(num_epochs):
  ...
  for step, (s_img, s_label, s_name) in enumerate(training_data_loader_s):
   ...
   if j < len(training_data_loader_s)-1:
    ...
    if (step) % 10 == 0:
        print ('steptwo2:Epoch [%d/%d],Step [%d/%d], Re_Loss: %.4f' %(epoch+1, num_epochs, step, len(training_data_loader_s)-1, Re_loss.data[0]))
  ...
  if (epoch) % 15 == 0:
    ...
    netZEn.save_network(netZEn, 'netZEn2', epoch, 'model2/')
    
    netDe.save_network(netDe, 'netDe2', epoch, 'model2/')
    Aclassifier.save_network(Aclassifier, 'Aclassifier2', epoch, 'model2/')
