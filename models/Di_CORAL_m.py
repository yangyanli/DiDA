...
class Di_CORAL_Model(BaseModel):
    def name(self):
        return 'Di_CORAL_Model'
    ...
    def initialize(self, opt):
        ...
        self.netZE = networks.define_ZE(opt.input_nc, opt.output_nc, opt.which_model_netZE, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        self.netAC = networks.define_AC(opt.output_nc, opt.input_nc, opt.which_model_netAC, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
    ...
    def set_input(self, opt, s_img, s_label, t_img):
        ...
        t_img1, t_img2 = torch.split(self.t_img, self.t_img.shape[0]/2, 0)
        E_t1, E_t2 = self.netSE(t_img1, t_img2)
        self.t_e = torch.cat((E_t1, E_t2), 0)
        #self.t_e = self.netSE(self.t_img)
        self.t_label = torch.max(self.t_e, 1)[1]
    ...
    def forward(self):
        self.code_s_s, self.code_s_t = self.netSE(self.s_img, self.t_img)
        self.code_s = torch.cat((self.code_s_s, self.code_s_t), 0)
        self.code_z = self.netZE(self.img_G)
        self.code = torch.cat((self.code_s, self.code_z), 1)
        self.Reimg = self.netDe(self.code).view(-1, 3, 28, 28)
    ...
    def combine(self, opt):
        ...
        self.code_s_source, code_s_target = torch.split(self.code_s, opt.batchSize/2, 0)
        code_z_source, self.code_z_target = torch.split(self.code_z, opt.batchSize/2, 0)
        code_s1_tar, code_s2_tar = torch.split(code_s_target, opt.batchSize / 4, 0)
        code_snew_tar = torch.cat((code_s2_tar, code_s1_tar), 0)
        code_mid = torch.cat((self.code_s_source, self.code_z_target), 1)#.contiguous()
        codenew = torch.cat((code_snew_tar, self.code_z_target), 1)#.contiguous()
